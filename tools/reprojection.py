	...
	def start_rendering(self):
		...
		for object_id in self.all_objects:
			...
			for i, stable_pose in enumerate(self.stable_poses):
				...
				projected_depth_im,new_camera_intr,table_height = self._projection(transformed_points,shifted_camera_intr)
...
#					full_map = full_map[:2,:]
					...
					c1_rotated = (np.dot(full_map,np.r_[c1.vector,[1]]) - np.array([320-48,240-48,0]))/3
					c2_rotated = (np.dot(full_map,np.r_[c2.vector,[1]]) - np.array([320-48,240-48,0]))/3
					print("C1",c1_rotated)
					print("C2",c2_rotated)
	...
	def _projection(self,transformed_points,camera_intr):
		...
		new_camera_intr.cx = 320
		new_camera_intr.cy = 240
